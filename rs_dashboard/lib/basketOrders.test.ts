...
test('resolveOrderRequest builds a Dhan fast-order request from a CE leg', () => {
  const leg = { side: 'S' as const, option: 'CE' as const, strike: 24000, qty: 75, type: 'MARKET' as const, underlying: 'NIFTY', productType: 'INTRADAY' as const };
...
test('resolveOrderRequest builds a Zerodha order request from a PE leg, snapping a limit price to the 0.05 tick', () => {
  const leg = { side: 'B' as const, option: 'PE' as const, strike: 24000, qty: 75, type: 'LIMIT' as const, price: 123.456, underlying: 'NIFTY', productType: 'INTRADAY' as const };
  ...
  assert.deepStrictEqual(req, {
    ...
    body: { tradingsymbol: 'NIFTY24721P24000', quantity: 75, side: 'BUY', orderType: 'LIMIT', exchange: 'NFO', product: 'MIS', price: 123.45 },
  });
});
...
test('resolveOrderRequest returns null when the strike exists but lacks the requested option side identifier', () => {
  const leg = { side: 'S' as const, option: 'PE' as const, strike: 24000, qty: 75, type: 'MARKET' as const, underlying: 'NIFTY', productType: 'INTRADAY' as const };
...
test('resolveOrderRequest builds a Dhan fast-order request for SENSEX with BSE_FNO segment', () => {
  const leg = { side: 'S' as const, option: 'CE' as const, strike: 81000, qty: 20, type: 'MARKET' as const, underlying: 'SENSEX', productType: 'INTRADAY' as const };
  const strikeMap = { '81000': { ceId: '55555', peId: '66666' } };
  const req = resolveOrderRequest('dhan', leg, strikeMap);
  assert.deepStrictEqual(req, {
    broker: 'dhan', url: '/api/scalper/fast-order',
    body: { securityId: '55555', quantity: 20, side: 'SELL', orderType: 'MARKET', exchangeSegment: 'BSE_FNO', productType: 'INTRADAY' },
  });
});

test('resolveOrderRequest builds a Zerodha order request for SENSEX with BFO exchange', () => {
  const leg = { side: 'B' as const, option: 'PE' as const, strike: 81000, qty: 20, type: 'MARKET' as const, underlying: 'SENSEX', productType: 'INTRADAY' as const };
  const strikeMap = { '81000': { ceSymbol: 'SENSEX24721C81000', peSymbol: 'SENSEX24721P81000' } };
  const req = resolveOrderRequest('zerodha', leg, strikeMap);
  assert.deepStrictEqual(req, {
    broker: 'zerodha', url: '/api/scalper/zerodha/order',
    body: { tradingsymbol: 'SENSEX24721P81000', quantity: 20, side: 'BUY', orderType: 'MARKET', exchange: 'BFO', product: 'MIS' },
  });
});

test('resolveOrderRequest maps productType MARGIN to Dhan productType MARGIN', () => {
  const leg = { side: 'S' as const, option: 'CE' as const, strike: 24000, qty: 75, type: 'MARKET' as const, underlying: 'NIFTY', productType: 'MARGIN' as const };
  const strikeMap = { '24000': { ceId: '12345', peId: '67890' } };
  const req = resolveOrderRequest('dhan', leg, strikeMap);
  assert.deepStrictEqual(req, {
    broker: 'dhan', url: '/api/scalper/fast-order',
    body: { securityId: '12345', quantity: 75, side: 'SELL', orderType: 'MARKET', exchangeSegment: 'NSE_FNO', productType: 'MARGIN' },
  });
});

test('resolveOrderRequest maps productType MARGIN to Zerodha product NRML', () => {
  const leg = { side: 'B' as const, option: 'PE' as const, strike: 24000, qty: 75, type: 'MARKET' as const, underlying: 'NIFTY', productType: 'MARGIN' as const };
  const strikeMap = { '24000': { ceSymbol: 'NIFTY24721C24000', peSymbol: 'NIFTY24721P24000' } };
  const req = resolveOrderRequest('zerodha', leg, strikeMap);
  assert.deepStrictEqual(req, {
    broker: 'zerodha', url: '/api/scalper/zerodha/order',
