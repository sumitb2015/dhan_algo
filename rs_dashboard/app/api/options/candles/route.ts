...
interface CacheEntry { data: CandleRow[]; ts: number; dataDate?: string; isToday?: boolean }
...
export async function GET(request: NextRequest) {
  ...
  if (hit && Date.now() - hit.ts < CACHE_TTL) {
    return NextResponse.json({ success: true, data: hit.data, dataDate: hit.dataDate, isToday: hit.isToday });
  }
  ...
  try {
    ...
    if (candles.length) cache.set(cacheKey, { data: candles, ts: Date.now(), dataDate, isToday });
    return NextResponse.json({ success: true, data: candles, dataDate, isToday });
  } catch (err) {
