...
interface CandleRow { time: string; 'CE LTP': number; 'PE LTP': number; Straddle: number }
interface CacheEntry { data: CandleRow[]; ts: number }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const parsed   = JSON.parse(jsonLine) as { candles?: CandleRow[]; error?: string };
    ...
    if (candles.length) cache.set(cacheKey, { data: candles, ts: Date.now() });
