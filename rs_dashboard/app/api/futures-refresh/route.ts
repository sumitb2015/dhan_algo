...
const SCRIPT_PATH  = path.join(PROJECT_ROOT, 'scripts', 'downloader', 'download_futures_manual.py');
const STATUS_FILE  = path.join(DEBUG_DIR, 'futures_refresh_status.json');
...
export interface FuturesRefreshStatus {
  running: boolean;
  done: boolean;
  message: string;
  error: string | null;
...
export async function GET(): Promise<NextResponse<FuturesRefreshStatus>> {
