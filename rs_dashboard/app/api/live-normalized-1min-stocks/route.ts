...
export async function GET() {
  ...
  if (result.error) {
    console.error('[/api/live-normalized-1min-stocks] spawn error:', result.error);
  ...
  try {
    ...
    if (!parsed.success) {
      console.error('[/api/live-normalized-1min-stocks]', parsed.error, (result.stderr ?? '').slice(0, 400));
    ...
    console.error('[/api/live-normalized-1min-stocks] parse error:', err, '\nstdout:', result.stdout);
