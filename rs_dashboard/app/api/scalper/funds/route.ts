...
export async function GET(): Promise<NextResponse> {
  try {
    const { stdout } = await execFileAsync(PYTHON_EXE, [SCALPER_SCRIPT, 'funds'], {
      cwd: PROJECT_ROOT,
      timeout: 20_000,
    });
    ...
    console.error('[/api/scalper/funds] error:', e.message, e.stderr ?? '');
    return NextResponse.json({ success: false, error: 'Failed to fetch funds', detail: String(e.message) }, { status: 500 });
  }
}
