...
export async function GET(): Promise<NextResponse> {
  try {
    const { stdout } = await execFileAsync(PYTHON_EXE, [SCALPER_SCRIPT, 'orders'], {
      cwd: PROJECT_ROOT,
      timeout: 20_000,
      windowsHide: true,
    });
    ...
    console.error('[/api/scalper/orders] error:', e.message, e.stderr ?? '');
    return NextResponse.json({ success: false, error: 'Failed to fetch orders', detail: String(e.message) }, { status: 500 });
  }
}
