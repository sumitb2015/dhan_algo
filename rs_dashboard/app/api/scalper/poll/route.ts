...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    console.error('[/api/scalper/poll] error:', e.message, e.stderr ?? '');
    return NextResponse.json({ success: false, error: 'Failed to poll data', detail: String(e.message) }, { status: 500 });
  }
}
