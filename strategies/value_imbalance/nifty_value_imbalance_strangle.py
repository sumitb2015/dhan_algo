...
class ValueImbalanceStrangle:
    def __init__(self, dry_run=True, initial_lots=1, max_lots=4,
                 ...
                 state_key="nifty_value_imbalance_strangle", broker="dhan"):
    ...
    def save_state(self, nifty_spot, ce_ltp, pe_ltp, total_pnl, status="RUNNING"):
        state_dict = {
            "strategy": "nifty_value_imbalance_strangle",
            "status": status,
    ...
    def run(self):
        ...
        while True:
            ...
            while True:
                ...
                ce_price, pe_price, curr_spot = self.fetch_ltps()
            ...
            while cycle_active:
                ...
                if diff_pct > active_thresh:
                    if winner_lots < self.max_lots:
                        ...
                        new_loser_lots = max(2, self.initial_lots)
                        ...
                        if new_strike:
                            ...
                            if exit_price > 0:
                                buy_oid = None
                                if not self.dry_run:
                                    buy_oid = self.broker.buy(old_strike, self.expiry, loser, old_loser_lots * self.nifty_lot_size)
                                    if not buy_oid:
    ...
    def find_rebalance_strike(self, option_type, target_value, lots, chain_df, spot=0.0):
        """Find the strike whose lots * price is closest to target_value.
...
""")
    ...
    STATE_KEY = f"nifty_value_imbalance_strangle_{args.instance_id}" if args.instance_id else "nifty_value_imbalance_strangle"
