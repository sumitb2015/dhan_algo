"""
Backtest v6: v4 + regime-exit (sell all when regime OFF 3 consecutive days)

New in v6:
  - When Nifty regime (close>50-DMA AND close>close_63d_ago) turns OFF for
    3+ consecutive trading days, sell ALL open positions at close.
  - Prevents holding through sustained corrections (e.g. March 2026).
  - Re-entry happens automatically on next regime-ON day per v4 rules.
  - All other v4 parameters unchanged.
"""
...
PORTFOLIO_SIZE     = 20
RS_LOOKBACK        = 126
MIN_RS             = 0.00        # no minimum RS cutoff for v6
SMA_PERIOD         = 50
MOM_PERIOD         = 63
SL_PCT             = 0.10
TARGET_PCT         = 0.30
BREAKEVEN_AT       = 0.15        # move SL to breakeven when peak +15%
TRAIL_AT           = 0.25        # trail 10% below peak when +25%
REBAL_HOLD         = 45
REBAL_TOP_N        = 30
SYM_COOLDOWN_DAYS  = 10
MAX_NEW_PER_DAY    = 5
REGIME_OFF_DAYS    = 3           # exit all after this many consecutive regime-OFF days
...
def build_tables(price_map):
    ...
    for sym, df in price_map.items():
        s50l[sym] = dict(zip(df["date"], df["close"].rolling(SMA_PERIOD,min_periods=SMA_PERIOD).mean()))
...
def run():
    print("="*72)
    print("NIFTY 50 RS STRATEGY v6  (v4 + regime-exit after 3 OFF days)")
    print("="*72)
    ...
    sl_hit:dict[str,int]={}
    rebal_month=-1
    consecutive_off=0   # track consecutive regime-OFF days
    regime_exited=False  # did we already do a regime-exit this OFF streak?

    def regime_on(d): return d in regime

    def entry_ok(sym,d,rs,day_idx):
        if rs<=MIN_RS: return False
        if day_idx-sl_hit.get(sym,-9999)<SYM_COOLDOWN_DAYS: return False
        s50=s50l[sym].get(d); row=pxl[sym].get(d)
    ...
    def record(sym,pos,exit_date,ep2,reason):
        ...
        if reason=="sl": sl_hit[sym]=trading_days.index(exit_date)
        del portfolio[sym]

    def do_buy(sym,d):
        row=pxl[sym].get(d)
        if not row: return None
        ep=float(row.close); sh=max(1,int(MAX_INVEST//ep))
        portfolio[sym]=Pos(sym,d,ep,sh); return portfolio[sym]
    ...
    for day_idx,day in enumerate(trading_days):
        ...
        # Normal SL / target exits (only when not doing regime-exit)
        ...
        if r_on and day.month!=rebal_month:
            ...
            for sym,pos in list(portfolio.items()):
                ...
                if held>REBAL_HOLD and sym not in top_n:
                    ...
                    pnl=pos.pnl(ep2)
                    print(f"  {day}: REBAL EXIT {sym} ({held}d, rank {next((i+1 for i,(s,_) in enumerate(ranked) if s==sym),'?')}) PNL Rs.{pnl:+,.0f} ({pnl/pos.invest*100:+.1f}%)")
                    record(sym,pos,day,ep2,"rebal")

        # Fill empty slots when regime ON
        if r_on:
    ...
    for sym,pos in portfolio.items():
        ...
        open_list.append({"symbol":sym,"entry_date":pos.entry_date,
                           ...
                           "unr_pct":unr/pos.invest*100,"sl":pos.sl,"target":pos.target})
