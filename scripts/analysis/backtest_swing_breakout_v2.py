"""
Multi-Period RS + Risk Parity + 2-Stage Chandelier Trailing Positional Swing Backtest (v2 - Rigorous Audit Edition).

Code Review & Audited Fixes Applied:
1. Exact Cash Accounting: Transaction costs (brokerage, STT, taxes, slippage) are strictly deducted from cash on every scale-out and exit.
2. Causality Protection: Trailing stop updates apply AFTER checking current day's stop (no intra-bar high-before-low lookahead bias).
3. Accurate Position Accounting: Trades are tracked at the position level to ensure win rate, trade count, and profit factor are not distorted by partial scale-outs.
4. Strict CAGR & Equity Curve Math: Compounded annual returns are calculated from exact cost-adjusted daily portfolio equity.
"""
...
def run_audited_backtest(cfg: SwingConfig, start: Optional[date] = None,
    ...
    for i, ts in enumerate(all_days[:-1]):
        ...
        for sym in list(book):
            ...
            # STEP C: Update High Water Mark & Trailing Stop AFTER checking today's exit
            pos.high_water = max(pos.high_water, hi)
            r_gain_after = (pos.high_water - pos.entry_price) / (cfg.risk_atr_mult * pos.atr_at_entry) if pos.atr_at_entry > 0 else 0
            if cfg.atr_trail_enabled and r_gain_after >= 2.0:
                trail_level = pos.high_water - cfg.trail_atr_mult * atr_v
                pos.stop = max(pos.stop, trail_level)
...
def print_audited_report(res: AuditResult):
    ...
    print("=" * 78)
    print("AUDITED POSITIONAL SWING BREAKOUT (V2)")
    print("=" * 78)
