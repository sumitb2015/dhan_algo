...
def main():
    ...
    # corrupt — each other's quotes/status/history/stop-trigger files.
    ...
    global QUOTES_FILE, HISTORY_FILE, STATUS_FILE, STOP_TRIGGER
    suffix = args.underlying.lower()
    QUOTES_FILE  = os.path.join(DEBUG_DIR, f'live_options_quotes_zerodha_{suffix}.json')
    HISTORY_FILE = os.path.join(DEBUG_DIR, f'live_options_history_zerodha_{suffix}.json')
    STATUS_FILE  = os.path.join(DEBUG_DIR, f'live_options_status_zerodha_{suffix}.json')
    STOP_TRIGGER = os.path.join(DEBUG_DIR, f'live_options_stop_zerodha_{suffix}.trigger')
