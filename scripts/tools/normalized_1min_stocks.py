"""
Fetch today's 1-min intraday candles for stocks and indexes and
print normalized (% change from each instrument's own previous close or session-open) series
as a single JSON line to stdout.

Usage:
    venv\\Scripts\\python.exe scripts/tools/normalized_1min_stocks.py
...
def _resolve(helper, symbol: str, kind: str):
    ...
    if kind == 'index':
        ...
        underlying = 'NIFTY' if symbol == 'NIFTY_FUT' else symbol
        sec = helper.find_future(underlying, exchange='NSE', instrument='FUTIDX')
        if not sec:
            return None
        return {
            'security_id': str(int(sec['SECURITY_ID'])),
        ...
        try:
            ...
            if sym_date:
                ...
                try:
                    ...
                    if prev_close:
                        baseline = prev_close
                        sys.stderr.write(f"[normalized_1min_stocks] {symbol} baseline: {baseline} (robust prev close prior to {sym_date})\n")
                    else:
                        sys.stderr.write(f"[normalized_1min_stocks] Warning: Could not get robust prev close for {symbol}, using session open.\n")
                except Exception as ex:
                    sys.stderr.write(f"[normalized_1min_stocks] Error calculating robust prev close for {symbol}: {ex}, using session open.\n")
